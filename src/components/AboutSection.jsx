...
const commands = [
  ...
  { type: 'out', text: 'Nguyen Thanh Nhan - Backend Developer', speed: 10, delayAfter: 400 },
    ...
    <section id="about" className="min-h-screen py-24 relative flex items-center justify-center">
      <div className="container mx-auto px-6 relative z-10 w-full" ref={containerRef}>
        ...
        <div className="text-center mb-28 flex flex-col items-center">
           ...
           >
             About <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-cyan-500">
               me
             </span>
           </motion.h2>
        </div>
            ...
            >
              ...
              <div className="p-5 md:p-8 h-[350px] md:h-[400px]">
                {commands.map((cmd, idx) => {
                  ...
                  return (
                    <div key={idx} className={`mb-5 ${cmd.type === 'cmd' ? 'text-indigo-600' : 'text-neutral-600'} whitespace-pre-wrap`}>
                      {cmd.type === 'cmd' ? (
                        <span className="text-[#0ea5e9] font-bold mr-3 tracking-widest">nahn_admin@~ %</span>
                      ) : (
